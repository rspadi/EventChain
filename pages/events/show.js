...
import { Card, Grid, Button, Form, Input, TextArea, Message, Segment, Divider, Modal, Header, Icon, Popup, Container } from 'semantic-ui-react';
...
class EventShow extends Component {
  ...
  static async getInitialProps (props) {
    ...
    console.log(props.query.address + ' - ' + details[0]);
  ...
  state = {
    title: this.props.iniDetails[0],
    description: this.props.iniDetails[1],
    location: this.props.iniDetails[2],
    fee: this.props.iniDetails[3],
    maxParticip: this.props.iniDetails[4],
    dateTime: moment.unix(this.props.iniDetails[5]),
    manager: this.props.iniDetails[6],
    eventCanceled: false,
  ...
  updateEventDetails = async () => {
    const { title, description, location, fee, maxParticip, dateTime, manager, eventCanceled,
      regParticipantCount, badRatingCount } = this.state;

    const details = await this.props.event.methods.getEventDetails().call();

    this.setState({eventCanceled: details[8]});
    this.setState({regParticipantCount: details[9]});
    this.setState({badRatingCount: details[10]});
  ...
  getContractProperties = async () => {
    ...
    const { fee, maxParticip, dateTime, manager, eventCanceled, regParticipantCount, badRatingCount, balance, userAddress,
      isRegistered, gaveBadRating, allowModify, allowCancel, allowBalance, allowRetrieve, allowRegister, allowUnregister,
      allowRate, allowUnrate, allowRefund, allowParticipList, userInfo } = this.state;
    ...
    const particip = await this.props.event.methods.participants(accounts[0]).call();
    this.setState({isRegistered: particip[4]});
    const badRate = await this.props.event.methods.badRating(accounts[0]).call();
    ...
    this.setState({allowModify: (moment().add(1, 'days') < dateTime && isOwner && !maxParticipReached && !eventCanceled ) ? true : false });
    ...
    this.setState({allowRegister: (moment().add(1, 'hours') < dateTime && !isOwner && !maxParticipReached && !isRegistered) ? true : false });
    this.setState({allowUnregister: (moment().add(6, 'hours') < dateTime && !isOwner && isRegistered) ? true : false });
    this.setState({allowRating: ( moment() > moment(dateTime).add(2, 'hours') && moment() < moment(dateTime).add(7, 'days') && !isOwner && isRegistered && !gaveBadRating) ? true : false });
    this.setState({allowUnrate: ( moment() > moment(dateTime).add(2, 'hours') && moment() < moment(dateTime).add(7, 'days') && !isOwner && isRegistered && gaveBadRating) ? true : false });
    this.setState({allowRefund: ( badRatingCount > (regParticipantCount/2) && !isOwner && isRegistered && gaveBadRating) ? true : false });
    ...
    if(isRegistered && !isOwner)
      uinfo += ' - You are alreday registered for this event';
    if(!isRegistered && !isOwner)
      uinfo += ' - You are not yet registered for this event';
    if(isRegistered && !isOwner && gaveBadRating)
      uinfo += ' - You provided a bad rating for this event';
  ...
  onModifyButtonClick = async () => {
    ...
    try{
      await this.props.event.methods.modifyEvent(this.state.title, this.state.description, this.state.location,
        this.state.fee, this.state.maxParticip, dateTimeUnix)
        .send({
          from: accounts[0],
          gas: '5000000'
      });

      this.setState({ loadingModify: false, showSuccessMessage: true, successMessage: 'The event details have been modified!' });
    	setTimeout(() => {
  ...
  onCancelButtonClick = () => { this.setState({ cancelModalOpen: true }) }
  onCancelModalClose = () => { this.setState({cancelModalOpen: false}); }
  onCancelModalClickNo = () => { this.setState({cancelModalOpen: false}); }
  onCancelModalClickYes = async () => {
    this.setState({ cancelModalOpen: false, loadingCancel: true, errorMessage: '' });
    const accounts = await web3.eth.getAccounts();

    try{
      await this.props.event.methods.cancelEvent()
        .send({
          from: accounts[0],
          gas: '5000000'
      });

      this.setState({ loadingCancel: false, showSuccessMessage: true, successMessage: 'This event has been canceled!' });
    	setTimeout(() => {
    	  this.setState({ showSuccessMessage: false, successMessage: '' })
    	}, 10000)
      this.updateEventDetails();
  ...
  onBalanceButtonClick = async () => {
    ...
    try{
      balance = await this.props.event.methods.contractBalance().call({ from: accounts[0] });
  ...
  onBalanceModalClose = () => { this.setState({balanceModalOpen: false}); }
  ...
  onParticipListButtonClick = async () => {
    ...
    try{
      //const regParticipants = await this.props.event.methods.getListOfRegisteredParticipants().call({from: accounts[0], gas: '5000000' });
      const regParticipants = await this.props.event.methods.getListOfRegisteredParticipants().call();
      const regParticipCount = regParticipants.length;

      const participList = await Promise.all(
        Array(parseInt(regParticipCount)).fill()
          .map((element, index) => {
            return this.props.event.methods.participants(regParticipants[index]).call();
          })
      );

      if(participList.length == 0)
        pList = 'Nobody has registered yet for this event.';

      for (var i = 0; i < participList.length; i++) {
          pList += 'Name: ' + participList[i][1] + ' - Email: ' + participList[i][2] + ' - Tel: ' + participList[i][3] + '\n';
      }
  ...
  onParticipListModalClose = () => { this.setState({participListModalOpen: false}); }
  ...
  onRetrieveButtonClick = async () => {
    ...
    try{
      await this.props.event.methods.retrieveEventFees()
        .send({
          from: accounts[0],
          gas: '5000000'
      });

      this.setState({ loadingRetrieve: false, showSuccessMessage: true, successMessage: 'Your funds have been retrieved!' });
    	setTimeout(() => {
    	  this.setState({ showSuccessMessage: false, successMessage: '' })
    	}, 10000)
      this.updateEventDetails();
  ...
  onRegisterButtonClick = () => { this.setState({ registerModalOpen: true }) }
  onRegisterModalClose = () => { this.setState({registerModalOpen: false, participName: '', participEmail: '', participTel: ''}); }
  onRegisterModalClickCancel = () => { this.setState({registerModalOpen: false, participName: '', participEmail: '', participTel: ''}); }
  onRegisterModalClick = async () => {
    this.setState({ registerModalOpen: false, loadingRegister: true, errorMessage: '' });
    const accounts = await web3.eth.getAccounts();

    try{
      await this.props.event.methods.registerForEvent(this.state.participName, this.state.participEmail, this.state.participTel)
        .send({
          from: accounts[0],
      ...
      this.setState({ loadingRegister: false, showSuccessMessage: true, successMessage: 'Thank you very much, you have been registered for this event!' });
    	setTimeout(() => {
    	  this.setState({ showSuccessMessage: false, successMessage: '' })
    	}, 10000)
      this.updateEventDetails();
  ...
  onUnregisterButtonClick =  () => { this.setState({unregisterModalOpen: true }) }
  onUnregisterModalClose = () => { this.setState({unregisterModalOpen: false}); }
  onUnregisterModalClickNo = () => { this.setState({unregisterModalOpen: false}); }
  onUnregisterModalClickYes = async () => {
    this.setState({ unregisterModalOpen: false, loadingUnregister: true, errorMessage: '' });
    const accounts = await web3.eth.getAccounts();

    try{
      await this.props.event.methods.unregisterFromEvent()
        .send({
          from: accounts[0],
          gas: '5000000'
      });

      this.setState({ loadingUnregister: false, showSuccessMessage: true, successMessage: 'You have been unregistered for this event!' });
    	setTimeout(() => {
    	  this.setState({ showSuccessMessage: false, successMessage: '' })
    	}, 10000)
      this.updateEventDetails();
  ...
  onRateButtonClick = async () => { this.setState({rateBadModalOpen: true}); }
  onRateBadModalClose = () => { this.setState({rateBadModalOpen: false}); }
  onRateBadModalClickNo = () => { this.setState({rateBadModalOpen: false}); }
  onRateBadModalClickYes = async () => {
    this.setState({ rateBadModalOpen: false, loadingRate: true, errorMessage: '' });
    const accounts = await web3.eth.getAccounts();

    try{
      await this.props.event.methods.rateEventAsBad()
        .send({
          from: accounts[0],
          gas: '5000000'
      });
  ...
  onUnrateButtonClick = async () => {
    ...
    try{
      await this.props.event.methods.reverseBadRating()
        .send({
          from: accounts[0],
          gas: '5000000'
      });

      this.setState({ loadingUnrate: false, showSuccessMessage: true, successMessage: 'Your rating has been reversed!' });
    	setTimeout(() => {
    	  this.setState({ showSuccessMessage: false, successMessage: '' })
    	}, 10000)
      this.updateEventDetails();
  ...
  onRefundButtonClick = async () => {
    ...
    try{
      await this.props.event.methods.recoverPaidFees()
        .send({
          from: accounts[0],
          gas: '5000000'
      });

      this.setState({ loadingRefund: false, showSuccessMessage: true, successMessage: 'Your fees have been refunded!' });
    	setTimeout(() => {
    	  this.setState({ showSuccessMessage: false, successMessage: '' })
    	}, 10000)
      this.updateEventDetails();
  ...
  render() {
    ...
    //const { address, details, eventFinished, canRetrieveFunds } = this.props;

    const { title, description, location, fee, maxParticip, dateTime, manager, eventCanceled,
      regParticipantCount, badRatingCount, balance, errorMessage, loadingModify, loadingCancel,
      loadingBalance, loadingRetrieve, loadingRegister, loadingUnregister, loadingRate, loadingUnrate,
      loadingRefund, registerModalOpen, cancelModalOpen, unregisterModalOpen, rateBadModalOpen,
      balanceModalOpen, participName, participEmail, participTel, showSuccessMessage, successMessage,
      isRegistered, gaveBadRating, allowModify, allowCancel, allowBalance, allowRetrieve, allowRegister,
      allowUnregister, allowRate, allowUnrate, allowRefund, allowParticipList, userInfo, participList } = this.state;
      ...
      <Layout>
        <Container text style={{ marginTop: '7em' }}>
          ...
          <Form error={!!errorMessage} success={showSuccessMessage}>
            ...
            <Message error icon='warning' header="There was a problem!" attached='bottom' negative content={this.state.errorMessage} />
            <Message success header={successMessage} />

          </Form>

          <Divider horizontal section>User Actions</Divider>
                ...
                <Segment color='teal' >
                  ...
                  <Grid >
                    ...
                    <Grid.Row columns={2} stretched>
                      <Grid.Column>
                        <Button disabled={!allowModify} loading={this.state.loadingModify} onClick={this.onModifyButtonClick} primary>Modify Event</Button>
                      </Grid.Column>
                      <Grid.Column>
                        <Button disabled={!allowCancel} loading={this.state.loadingCancel} onClick={this.onCancelButtonClick} negative>Cancel Event</Button>
                      </Grid.Column>
                    </Grid.Row>
                    <Grid.Row columns={2} stretched>
                      <Grid.Column>
                        <Button disabled={!allowBalance} loading={this.state.loadingBalance} onClick={this.onBalanceButtonClick} primary>Contract Balance</Button>
                      </Grid.Column>
                      <Grid.Column>
                        <Button disabled={!allowParticipList} loading={this.state.loadingParticipList} onClick={this.onParticipListButtonClick} primary>List of Participants</Button>
                      </Grid.Column>
                    </Grid.Row>
                    <Grid.Row columns={1} stretched>
                      <Grid.Column>
                        <Button disabled={!allowRetrieve} loading={this.state.loadingRetrieve} onClick={this.onRetrieveButtonClick} primary>Retrieve Event Fees</Button>
                      </Grid.Column>
                    </Grid.Row>
                  </Grid>
              ...
              <Grid.Column>
                <Segment color='teal'>
                  ...
                  <Grid>
                    ...
                    <Grid.Row columns={2} stretched>
                      <Grid.Column>
                        <Button disabled={!allowRegister} loading={this.state.loadingRegister} onClick={this.onRegisterButtonClick} primary>Register</Button>
                      </Grid.Column>
                      <Grid.Column>
                        <Popup
                        trigger={<Button disabled={!allowUnregister} loading={this.state.loadingUnregister} onClick={this.onUnregisterButtonClick} negative>Unregister</Button>}
                        content='Test' />
                      </Grid.Column>
                    </Grid.Row>
                    <Grid.Row columns={2} stretched>
                      <Grid.Column>
                        <Button disabled={!allowRate} loading={this.state.loadingRate} onClick={this.onRateButtonClick} negative>Rate Event as Bad</Button>
                      </Grid.Column>
                      <Grid.Column>
                        <Button disabled={!allowUnrate} loading={this.state.loadingUnrate} onClick={this.onUnrateButtonClick} primary>Delete Bad Rating</Button>
                      </Grid.Column>
                    </Grid.Row>
                    <Grid.Row loading={this.state.loadingRefund} onClick={this.onRefundButtonClick} columns={1} stretched>
                      <Grid.Column>
                        <Button disabled={!allowRefund} primary>Request a Refund</Button>
                      </Grid.Column>
                    </Grid.Row>
                  </Grid>
